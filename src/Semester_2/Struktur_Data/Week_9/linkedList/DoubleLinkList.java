...
class DoubleLink {
	...
	public boolean insertBefore(int dat, int dat1, double dat2) {
		// mencari data
		Node indek = first; // pointer untuk mencari data
		while (indek.Data1 != dat) { // cari data dahulu
			indek = indek.next; // pindah ke data berikutnya
			if (indek == null) { // data tidak ditemukan
				return false; // keluar dari method
			}
		}

		// data ditemukan dan data baru dimasukkan
		Node newNode = new Node(dat1, dat2); // membuat node baru
		if (indek == first) { // jika data yang dicari berada di awal list
			first = newNode; // node baru menjadi node pertama
		} else { // jika data yang dicari tidak berada di awal list
			newNode.previous = indek.previous; // node baru menunjuk ke node sebelumnya
			indek.previous.next = newNode; // node sebelumnya menunjuk ke node baru
		}

		newNode.next = indek; // node baru menunjuk ke node yang dicari
		indek.previous = newNode; // node yang dicari menunjuk ke node baru
		return true; // keluar dari method
    }
